  ...
  return (
    <div className={`flex flex-col items-center p-4 min-h-screen ${themeClasses.bg} ${themeClasses.text}`}>
      ...
      <div className="flex flex-row flex-wrap items-center justify-between w-full max-w-4xl gap-4 mb-6 no-print">
        ...
        <div className="flex items-center gap-4">
          ...
          <Select value={language} onValueChange={(value: "fr" | "en") => setLanguage(value)}>
            <SelectTrigger className={`min-w-[135px] w-auto`}>
              <SelectValue placeholder="Language" />
            </SelectTrigger>
            <SelectContent className={themeClasses.dropdownBg}>
          ...
          <Button variant="outline" size="sm" onClick={() => setScale(prev => Math.min(prev + 0.25, 2.0))}>
      ...
      {loading ? (
        ...
        <PDFViewer pdfUrl={cvUrls[language]!} language={language} scale={scale} />
      )}
    </div>
  )
}
